...
- 既定で **議論一覧** (`GET /api/flow/sessions`、discussion_paper を新しい順) を表示。
  **下書き(編集中・未確定) / 進行中(未収束) / 結論あり** を `state` バッジで区別して在庫表示。
- **「＋ 新規議論開始」** → 入力フォーム (テーマ + 議論タイプ=議論既定 + ターン数 + タグ) を表示。
- **送信すると即座にペーパー編集画面へ遷移** (草案生成中は「準備中…」) → 確定で議論が自動進行。
- 一覧の項目クリック:
  - **下書き** → ペーパー編集を**再開** (メモリに無くても `rehydrateDraftEntry` が draft 行から復元)。
  - **進行中/結論あり** → その議論の**ライブ表示**を開く (進行中はライブ更新 / 収束済みは最終状態)。
  各画面の **「← 議論一覧へ」** で一覧に戻る。

確定すると議論が自動進行し、**ペーパー + 各 LLM の意見がライブ描画され、ペーパーが更新されていく**
(§議論ライブ表示)。(`/chat` は別物の「フリーチャット」= 軽量チャットでその場議論。)

**保存タイミング (status による 1 行ライフサイクル)**: `discussion_paper.status` で管理する。
- **編集ゲートで草案が ready になった時点**で `persistDraftPaper` が status='draft' 行を作る
  (= ドラフトも一覧に出る/再開できる)。編集 (ブロック編集 / NL 調整 / 戻す) ごとに同 session 行を更新。
- **確定 (approve→runFlow の `persistPaper`)** で同 session 行を status='started' に **upsert**
  (重複行を作らない)。発話は毎ターン `flow_utterance`、収束は `flow_conclusion` で表す。
- = ペーパーを作り始めた時点から保存され、下書き→進行中→結論ありと 1 行が状態遷移する。
