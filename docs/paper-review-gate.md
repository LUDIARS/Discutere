...
トップ (`/`) の **「チャット議論」** カード → `/flow` (議論タイプ=議論を既定選択)。
`/flow` で **テーマ + 議論のターン数 (ラウンド/ターン) + タグ** を記入 → 送信すると
ペーパー編集ゲート (下記) に入り、**確定すると議論が自動進行**して
**ペーパー + 各 LLM の意見がライブ描画され、ペーパーが更新されていく** (§議論ライブ表示)。
(`/chat` は別物の「フリーチャット」= 軽量チャットでその場議論。)
