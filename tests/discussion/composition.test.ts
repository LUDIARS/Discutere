...
const CFG: PartyConfig = {
  ...
  keymanCount: 2,
  facilitatorModel: "claude-opus-5",
  keymanModel: "claude-opus-5",
  opinionModels: [
...
assert.ok(party.filter((m) => m.tier !== "opinion").every((m) => m.model === "claude-opus-5"), "司会/キーマンは Opus");
assert.ok(
...
assert.ok(
  swapped.filter((m) => m.tier === "keyman").every((m) => m.model === "claude-opus-5"),
  "入替後もキーマンは Opus"
);
