...
const dir = path.resolve(".tmp/p6-lineage");
...
core.repos.utterance.create({ workspaceId: ws, sessionId: sid, rawContent: "parry timing", postedAt: Date.now() } as any);
const r = submitMessage({ core, workspaceId: ws, sessionId: sid, rawContent: `/lineage ${mid}` });
assert.equal(r.commandResult?.ok, true);
assert.match(r.commandResult?.message ?? "", /Lineage/);
core.close();
console.log("cross lineage passed");
