...
const dir = path.resolve(".tmp/p6-find");
...
const r = submitMessage({ core, workspaceId: ws, sessionId: sid, rawContent: "/find dash" });
assert.equal(r.commandResult?.ok, true);
assert.match(r.commandResult?.message ?? "", /mechanic/i);
core.close();
console.log("cross find passed");
